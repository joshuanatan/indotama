...
create view v_brg_warehouse_aktif as 
select id_pk_brg_warehouse,brg_warehouse_qty,brg_warehouse_status,id_fk_brg,id_fk_warehouse,brg_nama 
from tbl_brg_warehouse 
inner join mstr_barang on mstr_barang.id_pk_brg = tbl_brg_warehouse.id_fk_brg
where brg_warehouse_status = 'aktif'
and brg_status = 'aktif'
order by id_fk_brg,id_fk_warehouse;
/*core script*/
...
delimiter $$
create procedure update_latest_stok_mstr_brg_kombinasi_warehouse(
in id_barang_utama_in int,
in id_fk_warehouse_in int,
out new_stok_in double
)
begin
	select floor(min(rasio_stok)) into new_stok_in
...
delimiter ;
	...
	drop procedure if exists list_barang_kombinasi_warehouse;
	delimiter $$
	create procedure list_barang_kombinasi_warehouse()
	begin
	...
	mstr_kombinasi_loop: LOOP
		...
		call update_latest_stok_mstr_brg_kombinasi_warehouse(id_barang_utama_var,id_warehouse_var,@new_stok);
		if finished = 1 then
			leave mstr_kombinasi_loop;
		end if;
