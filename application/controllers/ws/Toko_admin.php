...
class Toko_admin extends CI_Controller{
    ...
    public function content(){
        ...
        $this->load->model("m_toko_admin");
        $this->m_toko_admin->set_id_fk_toko($id_toko);
        $result = $this->m_toko_admin->content($page,$order_by,$order_direction,$search_key,$data_per_page);

        if($result["data"]->num_rows() > 0){
            $result["data"] = $result["data"]->result_array();
            for($a = 0; $a<count($result["data"]); $a++){
                $response["content"][$a]["id"] = $result["data"][$a]["id_pk_toko_admin"];
                $response["content"][$a]["id_toko"] = $result["data"][$a]["id_fk_toko"];
                $response["content"][$a]["id_user"] = $result["data"][$a]["id_fk_user"];
                $response["content"][$a]["status"] = $result["data"][$a]["toko_admin_status"];
                $response["content"][$a]["last_modified"] = $result["data"][$a]["toko_admin_last_modified"];
        ...
        if($check != ""){
            foreach($check as $a){
                ...
                if($this->form_validation->run()){
                    $toko_nama = $this->input->post("nama".$a);
                    $this->load->model("m_user");
                    $this->m_user->set_user_name($toko_nama);
                    ...
                    if($result->num_rows() > 0){
                        ...
                        $this->load->model("m_toko_admin");
                        if($this->m_toko_admin->set_insert($id_fk_toko,$id_fk_user,$toko_admin_status)){
                            if($this->m_toko_admin->insert()){
    ...
    public function update(){
        ...
        if($this->form_validation->run()){
            ...
            if($result->num_rows() > 0){
                $result = $result->result_array();
                $id_pk_toko_admin = $this->input->post("id");
                $id_fk_user = $result[0]["id_pk_user"];
    
                $this->load->model("m_toko_admin");
                if($this->m_toko_admin->set_update($id_pk_toko_admin,$id_fk_user)){
                    if($this->m_toko_admin->update()){
                        $response["msg"] = "Data is recorded to database";
                    }
                    else{
                        $response["status"] = "ERROR";
    ...
    public function delete(){
        ...
        if($id != "" && is_numeric($id)){
            $id_pk_toko_admin = $id;
            $this->load->model("m_toko_admin");
            if($this->m_toko_admin->set_delete($id_pk_toko_admin)){
                if($this->m_toko_admin->delete()){
